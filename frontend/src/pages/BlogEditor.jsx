...
const BlogEditor = ({ addPost }) => {
  ...
  const imageHandler = useCallback(() => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        // ✅ 업로드된 이미지 URL에 localhost:8080 추가
        const uploadedImageUrl = `http://localhost:8080${response.data}`;
  ...
  const handlePostSubmit = async () => {
    ...
    try {
      ...
      const uploadedThumbnailUrl = `${uploadResponse.data}`;
